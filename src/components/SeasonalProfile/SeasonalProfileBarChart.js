  ...
  render () {
    ...
    return (
      <g transform={`translate(${x},${y})`}>
        <text x={width / 2} y={0} dy={-40} textAnchor="middle" fill="#96b633" fontWeight="700" fontSize="2rem">{formatEuros(data?.preu[index].euros)}</text>
        <text x={width / 2} y={0} dy={-15} textAnchor="middle" fill="#666" fontWeight="700" fontSize="2rem">{formatkWh(data?.valorKwh[index].kWh)}</text>
      </g>
    )
  }
}
    ...
    <div style={{ height: '300px' }}>
      <ResponsiveContainer>
        <BarChart width={730} height={250} data={data?.preu}>
          ...
          <XAxis dataKey="season" tick={{ transform: 'translate(0, 8)' }} tickFormatter={ tickItem => tickItem.toUpperCase() } tickLine={false} fontWeight={500} fontSize="1.75rem" />
