...
const CustomLabel = (props) => {
  ...
  return (
    <g transform={`translate(${x},${y})`}>
      <text x={width / 2} y={0} dy={-40} textAnchor="middle" fill="#96b633" fontWeight="600" fontSize="1.5rem">{formatEuros(data?.price[index].euros)}</text>
      <text x={width / 2} y={0} dy={-15} textAnchor="middle" fill="#666" fontWeight="600" fontSize="1.5rem">{formatkWh(data?.valueKwh[index].kWh)}</text>
    </g>
  )
}
